  ...
  return (
    ...
    >
      <div className="mb-10 mt-6">
        <Link to="/" className="inline-flex items-center space-x-2 text-slate-400 hover:text-primary transition-colors font-medium group mb-6">
          <ArrowLeft size={20} className="group-hover:-translate-x-1 transition-transform" />
          <span>Back to Shop</span>
        </Link>
        <h2 className="text-5xl font-black text-white capitalize tracking-tight">{name}</h2>
        <p className="mt-4 text-lg text-slate-400 font-medium">
          Showing <span className="text-white">{filteredProducts.length}</span> premium products
        </p>
      </div>
      ...
      >
        {filteredProducts.map(product => (
          <motion.div 
            variants={item}
            key={product.id} 
            className="group bg-slate-800/40 backdrop-blur-sm border border-slate-700/50 rounded-3xl overflow-hidden hover:bg-slate-800/80 transition-all duration-300 hover:shadow-2xl hover:shadow-primary/10 hover:-translate-y-1 flex flex-col"
          >
      ...
      {filteredProducts.length === 0 && !isLoading && (
        <motion.div 
          ...
          className="text-center bg-slate-800/40 backdrop-blur-md border border-slate-700/50 rounded-3xl py-24 mt-8 flex flex-col items-center justify-center"
        >
          ...
          <p className="text-slate-400 text-lg mb-8 max-w-md">We couldn't find any products in the "{name}" category at the moment.</p>
